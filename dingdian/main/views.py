...
def index():
    ...
    books = Novel.query.filter_by(book_shelf=1, page=page).all()
    return render_template('index.html', form=form, page=page, books=books)
...
@main.route('/shelf/<int:book_id>/<int:status>')
def shelf(book_id,status):
    book = Novel.query.filter_by(id=book_id).first()
    book.book_shelf = status
    db.session.commit()
